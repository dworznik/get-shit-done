...
function loadConfig(cwd) {
  ...
  try {
    ...
    return {
      ...
      codex_supervisor_phase: get('codex_supervisor_phase', { section: 'workflow', field: 'codex_supervisor_phase' }) ?? defaults.codex_supervisor_phase,
      ...
      node_repair: get('node_repair', { section: 'workflow', field: 'node_repair' }) ?? true,
      node_repair_budget: get('node_repair_budget', { section: 'workflow', field: 'node_repair_budget' }) ?? 2,
      ui_phase: get('ui_phase', { section: 'workflow', field: 'ui_phase' }) ?? true,
      ui_safety_gate: get('ui_safety_gate', { section: 'workflow', field: 'ui_safety_gate' }) ?? true,
      text_mode: get('text_mode', { section: 'workflow', field: 'text_mode' }) ?? defaults.text_mode,
      research_before_questions: get('research_before_questions', { section: 'workflow', field: 'research_before_questions' }) ?? false,
      discuss_mode: get('discuss_mode', { section: 'workflow', field: 'discuss_mode' }) ?? 'discuss',
      skip_discuss: get('skip_discuss', { section: 'workflow', field: 'skip_discuss' }) ?? false,
